...
function defaultGameBoard(): Piece[] {
...
export class GameBoard {
    ...
    public copy(): GameBoard {
        return new GameBoard(this.pieces, this.currentState.copy(), true);
    }
    ...
    private captureIndices(index: number): [number, number][] {
        let piece: Piece = this.pieces[index];
        return utils.zip(...utils.divvy(potentialMoveFunctions(piece).map(f => f(index))))
    ...
    move(sourceIndex: number, targetIndex: number): GameBoard | null {
        ...
        if(this.currentState.isRegularTurn()) {
            ...
            let newGameBoard: GameBoard = new GameBoard(this.pieces, this.currentState, true);
            for(let i = 0; i < captureMoves.length; i++) {
                ...
                if (currentTarget === targetIndex) {
                    ...
                    newGameBoard.jump(sourceIndex, jumpOverIndex, targetIndex);
                    moveMade = true;
                    // If the current piece can jump again, we return a Multicapture.
                    if(newGameBoard.findAllCaptures(targetIndex).length != 0) { 
                        newGameBoard.currentState = new Multicapture(this.currentState.color, targetIndex);
                        return newGameBoard;
                    }
                    // And since there was a capture, we check to see if it's game over.
                    if(newGameBoard.isGameOver()) {
                        newGameBoard.currentState = new CompleteGame(this.currentState.color)
                        return newGameBoard;
                    }
                }
            }
            if(!moveMade) {
                ...
                newGameBoard.pieces[sourceIndex] = Piece.NONE;
                newGameBoard.pieces[targetIndex] = this.pieces[sourceIndex];
            }
            ...
            if(promotableLocation(this.currentState.color, targetIndex)) {
                newGameBoard.promote(targetIndex);
            }
            // And now we give the turn over to the other player.
            newGameBoard.currentState = new RegularTurn(otherColor(this.currentState.color));
            return newGameBoard;
        }
        if(this.currentState.isMulticapture()) {
            ...
            // Otherwise, it's a good move.
            let newGameBoard = new GameBoard(this.pieces, this.currentState, true);
    ...
    public finishEarly(color: Color): GameBoard {
        return new GameBoard(this.pieces, new CompleteGame(otherColor(color)), true);
    }
}
